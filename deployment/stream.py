...
@inject_server_info
async def camera_capture(cap,channel,stop_event,interpreter,label,camera_info,**kwargs):
    i = 0
    # buffer = [""] * 8
    total = 0
    inference_total = 0
    ...
    print("connection: ",channel,camera_info["tid"])
    ...
    while(cap.isOpened()):
        ...
        if retval:
            ...
            if camera_info["inference"]!=0 and camera_info["inference_settings"] == 0:
                outputs, obtainable = camera_info["inference"].inference(image,interpreter, label,camera_info)
            else:
                if camera_info["inference_settings"] == 0:
                    outputs, obtainable = edge.inference(image,interpreter, label,camera_info)
                if camera_info["inference_settings"] == 1:
                    outputs = interpreter.inference(image)
                    if camera_info["inference"]!=0:
                        outputs, obtainable = camera_info["inference"].post_process(outputs,label,interpreter.anchors,camera_info['threshold'])
                    else:
                        outputs, obtainable = edge.post_process(outputs,label,interpreter.anchors,camera_info['threshold'])
                        ...
                        if len(share_outputs[stream]) > 0:
                            ...
                            if abs(time.time()-share_outputs[stream][-1][1]/1000) < 2.5/camera_info["FPS"]:
                                if camera_info["share_view"]:
                                    ...
                                    if camera_info["inference"] != 0:
                                        response_text = camera_info["inference"].process_input(publisher_output[0],outputs,obtainable)
                                    else:
...
def stream_redis(r,edge_addr,cameras,thread_key,thread_list,mainkey):
    # cap_list = {}
    ...
    for i in (cameras.keys()):
        print("stream from camera",i,"with ",cameras[i]["FPS"], " fps")
        if cameras[i]["inference_settings"] == 0:
        ...
        if len(cameras[i]["listen"])>0:
            subcriber_listen = threading.Thread(target=loop.run_until_complete,args=(edge.listen_others(cameras[i]["listen"],stop_event,share_outputs),))
            subcriber_listen.start()
            thread_list.append(subcriber_listen)
        _thread = threading.Thread(target=loop.run_until_complete, args=(camera_capture(cap,channel,stop_event,interpreter,label,cameras[i]),))
        _thread.start()
